...
void timer(int i)
{
	side_movement = Normalize(CrossProduct(VectorSub(cam, lookAtPoint),upVector));
	if (glutKeyIsDown('s'))
	{
		vec3 a = VectorSub(cam, lookAtPoint);
		cam = VectorAdd(cam, Normalize(a));
	}
	if (glutKeyIsDown('w'))
	{
		vec3 a = VectorSub(cam, lookAtPoint);
		cam = VectorSub(cam, Normalize(a));
	}
	if (glutKeyIsDown('a'))
	{
